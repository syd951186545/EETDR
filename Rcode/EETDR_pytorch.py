...
class EETDR:
    ...
    def __init__(self, TensorX, X, Y, Z, core_G, initU, initI, initA):
        ...
        # factors[1], factors[2]
        self.G = tr.from_numpy(core_G)
        self.U = tr.from_numpy(initU)
        self.I = tr.from_numpy(initI)
        self.A = tr.from_numpy(initA)
    ...
    def eetdr_SGDonce(self):
        ...
        #################################################################################################
        ######更新核张量G,近似张量X，求重构误差
        self.U, self.I, self.A = tr.cat((self.U1, self.U2), axis=1), \
                                 tr.cat((self.I1, self.I2), axis=1), \
                                 tr.cat((self.A1, self.A2), axis=1)
...
if __name__ == "__main__":
    ...
    pathdir = "E:/ALLworkspace/Pyworkplace/EETDR"
    ...
    if already == 0:
        initU = np.load(pathdir + "/result//initU.npy")
        initI = np.load(pathdir + "/result//initI.npy")
        initA = np.load(pathdir + "/result//initA.npy")
    else:
        initU = np.load(pathdir + "/result//reU" + str(already) + ".npy")
        initI = np.load(pathdir + "/result//reI" + str(already) + ".npy")
        initA = np.load(pathdir + "/result//reA" + str(already) + ".npy")
    logfile = open(pathdir + "/result/log/log_fast.txt", "w")
    #################################################################
    print(initA.shape)
