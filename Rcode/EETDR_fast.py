...
class EETDR:
    ...
    def eetdr_SGDonce(self, TensorX_approximation):
        ...
        for minibatch in range(50):
            print("小批量次数{}".format(minibatch))
            ##############################################################################################
            # U2的随机梯度下降更新########################################################################
            seed = np.random.randint(0,self.num_data)
            i_once = self.X[seed][0]  # 随机梯度下降， 随机选取第i个用户更新
            j_once = self.X[seed][1]  # 随机梯度下降， 随机选取第i个用户更新
            k_once = self.X[seed][2]  # 随机梯度下降， 随机选取第i个用户更新
            rr1 = self.r + self.r1
            rr2 = self.r + self.r2
            rr3 = self.r + self.r3
            Ur = self.r1 + self.r
...
if __name__ == "__main__":
    ...
    while flag:
        count += 1
        logfile = open(pathdir + "/result/log/log_fast_300.txt", "a")
